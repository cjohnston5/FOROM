...
***               contact: craigmtjohnston@gmail.com                   ***
**************************************************************************
**************************************************************************
**************************************************************************
**************************************************************************
*             This is the fourth step in model calibration
**************************************************************************
**************************************************************************
* Step 1: Condence FAOSTAT into user defined number of regions
* Step 2: Reconcile FAOSTAT data by correcting production numbers,
*        calculates IO coefficients, and supply and demand prices
* Step 3: Calibrate trade model to replicate observed data using PMP
* Step 4: Run calibrated model for policy/scenario analysis
**************************************************************************
**************************************************************************

$if not setglobal SC  $setglobal SC SSP3
...
*******************************************************************************
SETS
*        k       *products                                /FWCI, FWNCI, FWC, FWNC, ORWC, ORWNC, IRC, IRNC, SWC, SWNC, PW, PB, FB, MP, CP, OP, WP, News, PWP, Opap, CPR, PEL/
*        k1(k)   *subset of manufactured products         /FWCI, FWNCI, FWC, FWNC, ORWC, ORWNC, IRC, IRNC, SWC, SWNC, PW, PB, FB, MP, CP, OP, WP, News, PWP, Opap, CPR, PEL/
...
*        f(k)    *subset of final products                /FW, ORW, SWC, SWNC, PW, PB, FB, News, PWP, Opap, PEL/
*        f(k)    *subset of final products                /FWC, FWNC, ORWC, ORWNC, SWC, SWNC, PW, PB, FB, News, PWP, Opap, PEL/
...
$ontext
****** regionalize GDP and Population for RPA regions from aggregate US data
GDP(rpa,t,scenario) = GDP('United States',t,scenario)*RPAgdp(rpa,'GDP share')/100;
RPApop_total = sum(rpa, RPApop(rpa,'population'));
POP(rpa,t,scenario) = POP('United States',t,scenario)*RPApop(rpa,'population')/RPApop_total;
$offtext
...
*RPAarea_nat = sum(rpa, RPAarea(rpa,'Natural17'));
*Area(rpa,'C',scenario,'Natural','2015') = Nat('United States','2015')*(RPAarea_total('C')/(sum(species, RPAarea_total(species))))*(RPAarea(rpa,'Natural17')/RPAarea_nat);
*Area(rpa,'NC',scenario,'Natural','2015') = Nat('United States','2015')*(RPAarea_total('NC')/(sum(species, RPAarea_total(species))))*(RPAarea(rpa,'Natural17')/RPAarea_nat);
...
*beta(g,'NC','2015')$(sum(SSP, Area(g,'NC',SSP,'total','2015')) gt 0) = sum(SSP, Area(g,'NC',SSP,'planted','2015'))/sum(SSP, Area(g,'NC',SSP,'total','2015'));
*beta(g,'C','2015')$(sum(SSP, Area(g,'C',SSP,'total','2015')) gt 0) = sum(SSP, Area(g,'C',SSP,'planted','2015'))/sum(SSP, Area(g,'C',SSP,'total','2015'));
...
**Note:Gscale. In the paper of Prakash Nepal,Jaana Korhonen etc, the authors translated
**projected increases in planted forest area into corresponding adjustments in forest stock growth rates for each country in GFPM
**Gscale represnts how many times highter stock growth in planted forests is as compared to the stock growth in natural forests


**Note: Import the NPP data
...
Y(c,k1) = Y(c,k1)/1000;
Q(c,k1) = Q(c,k1)/1000;
pd(c,k1) = pd(c,k1)*1000;
ps(c,k1) = ps(c,k1)*1000;
ps(c,k1)$(ps(c,k1) lt 25) = ps('South',k1);

bs(c,k1)$(Y(c,k1) gt 1) = ps(c,k1)/(pes(c,k1)*Y(c,k1));
bs(c,k1)$(Y(c,k1) le 1) = 1000;
as(c,k1)$(Y(c,k1) gt 0) = ps(c,k1) - bs(c,k1)*Y(c,k1);
as(c,k1)$(Y(c,k1) eq 0) = ps(c,k1) - bs(c,k1);
*as(c,k1) = 50;
*bs(c,k1)$(Y(c,k1) gt 0 AND PS(c,k1) gt 50) = (ps(c,k1) - as(c,k1))/Y(c,k1);
*bs(c,k1)$(PS(c,k1) le 100) = bs('South',k1);

bd(c,f)$(Q(c,f) gt 1) = -1*pd(c,f)/(ped(c,f)*Q(c,f));
bd(c,f)$(Q(c,f) le 1) = 1000;
ad(c,f) = pd(c,f) + bd(c,f)*Q(c,f);
ad(c,f)$(Q(c,f) eq 0) = pd(c,f) + bd(c,f);
$offtext
...
*Y(c,k1)$(Y(c,k1) eq 0) = .001;
*Q(c,k1)$(Q(c,k1) eq 0) = .001;
...
*bd(g,f)$(Q(g,f) gt 0) = -1*pd(g,f)/(ped(g,f)*Q(g,f));
*bd(g,f)$(Q(g,f) le 0) = 100000;
*ad(g,f)$(Q(g,f) gt 0) = pd(g,f) + bd(g,f)*Q(g,f);

bd(c,f)$(Q(c,f) gt 1) = -1*pd(c,f)/(ped(c,f)*Q(c,f));
avgbd(f) = sum(c, bd(c,f)$(Q(c,f) gt 1))/card(c);
bd(c,f)$(Q(c,f) le 1) = avgbd(f);
ad(c,f) = pd(c,f) + bd(c,f)*Q(c,f);
*ad(g,f) = pd(g,f)*(ped(g,f) - 1)/ped(g,f);


*=== End IMPORT NPP DATA FROM SPREADSHEET ===***************************************************************************************************************************************************************
*bs('Rocky Mountain','IRNC') = sum(rpa$(ord(rpa) gt 4),bs(rpa,'IRC'));
*=== End IMPORT NPP DATA FROM SPREADSHEET ===***************************************************************************************************************************************************************
...
* Material balance
*Eq1(g,f)..         qs(g,f) + sum(gg, flow(gg,g,f)) - sum(gg, flow(g,gg,f)) =e= qd(g,f);
Eq1(g,k)..         qs(g,k) + sum(gg, flow(gg,g,k)) - sum(gg, flow(g,gg,k)) =e= qd(g,k) + sum(n, recov(g,k,n)*qs(g,n));
*Eq1(g,f)..         qs(g,f) + sum(gg, flow(gg,g,f)) - sum(gg, flow(g,gg,f)) =e= qd(g,f);
*Eq2(g,w)..         qs(g,w) + sum(gg, flow(gg,g,w)) - sum(gg, flow(g,gg,w)) =e= sum(f, recov(g,w,f)*qs(g,f));
...
*Eq11(g,r)..        Y(g,r) - sum(j, ru(r,j)*Y(g,j)) =L= 0;
...
********************************************************************
*         Inventory(g,species) = Stock(g,species,t);

*         qs.lo(rpa,h)$(Johnremoval(t,rpa,h) gt 0.001) = Johnremoval(t,rpa,h)*.98;
*         qs.up(rpa,h)$(Johnremoval(t,rpa,h) gt 0.001) = Johnremoval(t,rpa,h)*1.05;
*         qs.l(rpa,h)$(Johnremoval(t,rpa,h) gt 0.001) = Johnremoval(t,rpa,h);
...
*        ad(g,f)$(qd.l(g,f) gt 0) = ad(g,f)*(1 + sum(SSP, gGDPP(g,t,SSP))*ied(g,f)/5);
         qd1(f,g,t+1) = qd1(f,g,t)*(1 + sum(SSP, gGDPP(g,t,SSP))*ied(g,f));
*         qd1('News',g,t+1) = qd1('News',g,t)*(1 + sum(SSP, GDPP(g,t,SSP))*(-0.000009) + sum(SSP, GDPP(g,t,SSP))*sum(SSP, GDPP(g,t,SSP))*0.00000000008);

*         ad(g,f)$(ped(g,f) gt 0) = priceD1(f,g,t)*1000*(ped(g,f) - 1)/ped(g,f);
**         bd(g,f)$(qd1(f,g,t+1) gt 0) = -1*priceD1(f,g,t)*1000/(ped(g,f)*qd1(f,g,t+1));
*         bd(g,f)$(qd1(f,g,t+1) gt 0) = (ad(g,f)-priceD1(f,g,t)*1000)/qd1(f,g,t+1);
*         ad(g,f)$(qd1(f,g,t+1) gt 0) = priceD1(f,g,t)*1000 + bd(g,f)*qd1(f,g,t+1);

         ad(g,f)$(qd1(f,g,t+1) gt 0 and (not ord(f)=1 ) and (not ord(f)=2) and (not ord(f)=13)) = priceD1(f,g,t)*1000 + bd(g,f)*qd1(f,g,t+1);
*         qd1('News',g,t+1) = qd1('News',g,t)*(1 - 0.05*4);
*         ad(g,'News') = ad(g,'News')*(1 - 0.035*4);
         ad(g,'News') = ad(g,'News')*(1 - sum(SSP, gGDPP(g,t,SSP))*4);

*         bd(g,f)$(qd1(f,g,t+1) gt 0 and (ord(f)=1)) = (ad(g,f)-priceD1(f,g,t)*1000)/qd1(f,g,t+1);
*         bd(g,f)$(qd1(f,g,t+1) gt 0 and (ord(f)=2)) = (ad(g,f)-priceD1(f,g,t)*1000)/qd1(f,g,t+1);
*         bd(g,f)$(qd1(f,g,t+1) gt 0 and (ord(f)=13))= (ad(g,f)-priceD1(f,g,t)*1000)/qd1(f,g,t+1);
...
* -------------- Exogenous shock to forest stock -------------------------------
*** Think about adding climate change here as change in the growth of NPP to the end of the equation below here
...
*        gStock(m,species,'planted',t)$(NOT tinit(t) AND sum(SSP, Area(m,species,SSP,'planted',t-1)) eq 0 AND (Stock(m,species,t-1) eq 0)) = 0.01;
...
*         qs1(h1,g,t+1)$(Stock(g,'C',t) gt 0 ) = qs1(h1,g,t)*(1 + sum(SSP, gArea(g,'C',SSP,'total',t))*1.5 + ((Stock(g,'C',t+1) - Stock(g,'C',t))/Stock(g,'C',t))*1.8 );
*         qs1(h2,g,t+1)$(Stock(g,'NC',t) gt 0 ) = qs1(h2,g,t)*(1 + sum(SSP, gArea(g,'NC',SSP,'total',t))*1.5 + ((Stock(g,'NC',t+1) - Stock(g,'NC',t))/Stock(g,'NC',t))*1.8 );
...
*         bs(rpa,k1)$(qs1(k1,rpa,t+1) gt 0.001 ) = priceS1(k1,rpa,t)*1000/(pes(rpa,k1)*qs1(k1,rpa,t+1));
*         bs(g,k1)$(qs1(k1,g,t+1) gt 1) = priceS1(k1,g,t)*1000/qs1(k1,g,t+1);
*         as(rpa,k1)$(qs1(k1,rpa,t+1) gt 0.001 ) = priceS1(k1,rpa,t)*1000 - bs(rpa,k1)*qs1(k1,rpa,t+1);
...
*****    Example scenario of increased Swedish demand for pellets
*        ad('Sweden','PEL')$(qd.l('Sweden','PEL') gt 0) = ad('Sweden','PEL')*1.02;
*        ad(c,'PEL')$(qd1("PEL",c,t) gt 0.001) = ad(c,'PEL')*(1+sum(SSP,Bdemand(t,SSP)));
...
*deltaps('Harvest',rpa,'C','2030')= (deltaps('Harvest',rpa,'C','2025') + deltaps('Harvest',rpa,'C','2035'))/2 ;
*deltaps('Harvest',rpa,'NC','2030')= (deltaps('Harvest',rpa,'NC','2025') + deltaps('Harvest',rpa,'NC','2035'))/2 ;
*deltaqs('Harvest',rpa,'C','2030')= (deltaqs('Harvest',rpa,'C','2025') + deltaqs('Harvest',rpa,'C','2035'))/2 ;
*deltaqs('Harvest',rpa,'NC','2030')= (deltaqs('Harvest',rpa,'NC','2025') + deltaqs('Harvest',rpa,'NC','2035'))/2 ;


avgpriced(k,t) = sum(g, priceD1(k,g,t))/countries;
avgprices(k,t) = sum(g, priceS1(k,g,t))/countries;
*priceD1(k,g,t)$(ABS(priceD1(k,g,t) - avgpriced(k,t-1)) gt 0.35*avgpriced(k,t-1)) = avgpriced(k,t-1)*1.05;
*priceS1(k,g,t)$(ABS(priceS1(k,g,t) - avgprices(k,t-1)) gt 0.35*avgprices(k,t-1)) = avgprices(k,t-1)*1.05;
...
PARAMETERS
         ...
         rpaharvest(c,t)
;
*check1(g,t) = Stock(g,'C',t)*0.5 - sum(h1, qs1(h1,g,t))/1000;
*check2(g,t) = Stock(g,'NC',t)*0.5 - sum(h2, qs1(h2,g,t))/1000;
...
*execute_unload "Results.gdx" qd1, qs1, priced1, prices1, Stock, Area, import, export, deltaps, deltaqs, check1, check2, as1,bs1, ad1,bd1,beta, gstock, cornifer, noncornifer;
*execute 'gdxxrw.exe Results.gdx Output=Output.xls epsout=0 par=qd1 rng=Demand!a1 par=qs1 rng=Supply!a1 par=priced1 rng=PriceD!a1 par=prices1 rng=PriceS!a1 par=Stock rng=Stock!a1 par=Area rng=Area!a1 par=import rng=Import!a1 par=export rng=Export!a1 par=deltaps rng=PCHANGE!a1 par=deltaqs rng=QCHANGE!a1 par=check1 rng=check1!a1 par=check2 rng=check2!a1 par=as1 rng=interceptS!a1 par=bs1 rng=Slope!a1 par=beta rng=PlantedShare!a1 par=gstock rng=Stockgrowth!a1  par=ad1 rng=Demandintercept!a1 par=bd1 rng=DemandSlope!a1 par=cornifer rng=Charvest!a1 par=noncornifer rng=NCharvest!a1 '
...
*QS_country(mm,regions, k1,tt)$ mapC2R(mm,regions) =sum(mapC2R(mm,regions),qs1(k1,mm,tt));
*QD_country(mm,regions,  f,tt)$ mapC2R(mm,regions) =sum(mapC2R(mm,regions),qd1(f, mm,tt));

QS_country(g,region, k1,tt)$ mapC2R(g,region)  = sum(mapC2R(g,region),qs1(k1,g,tt));
QD_country(g,region,  f,tt)$ mapC2R(g,region)  = sum(mapC2R(g,region),qd1(f, g,tt));
QS_region(k1,region,tt)=sum(g,QS_country(g,region, k1,tt));
QD_region( f,region,tt)=sum(g,QD_country(g,region,  f,tt));
...
QD_ALL(f,g,tt)=qd1(f, g,tt);
QD_ALL(f,region,tt)=QD_region(f, region,tt);
QD_ALL(f,"World",tt)=sum(g,qd1(f,g,tt));
QD_ALL(f,"United States",tt)=sum(RPA,qd1(f,RPA,tt));
...
*QD_ALL(k,allcnr,tt)$(allcnr$region)=QD_region(k, region,tt);

*Forest_Harvest("C",allcnr,tt)=cornifer(allcnr,'C',tt);
*Forest_Harvest("NC",allcnr,tt)=noncornifer(allcnr,'NC',tt);
*Forest_Harvest("T",allcnr,tt)= cornifer(allcnr,'C',tt)+ noncornifer(allcnr,'NC',tt);
...
*Forest_Harvest_country(g,region,species,tt)$mapC2R(g,region)= sum(mapC2R(g,region),Area(g,species,scenario,type,tt) );
*Forest_Harvest_region(region,species,tt)=sum(g,Forest_Harvest_country(g,region,species,tt));
...
display Forest_landarea,Forest_Harvest,Forest_stock;
...
display QS_ALL, QD_ALL,g,mm,check1,check2;

execute_unload "ForStep5.gdx" QS_ALL, QD_ALL, NetImport_ALL, NetExport_ALL;
...
loop (f,put / f.te(f) Loop (tt, put  tt.val );
      loop(allcnr, put / allcnr.tl;
      loop(tt, put  QD_ALL(f,allcnr,tt) ););  put /)

put / "Production Price" /;
loop (k2,put / k2.te(k2) Loop (tt, put  tt.val );
      loop(allcnr, put / allcnr.tl;
      loop(tt, put  PS_ALL(k2,allcnr,tt) ););  put /)

put / "Consumption Price" /;
loop (f1,put / f1.te(f1) Loop (tt, put  tt.val );
      loop(allcnr, put / allcnr.tl;
      loop(tt, put  PD_ALL(f1,allcnr,tt) ););  put /)
...
*************************************************************************************************
QS_country(*,regions,*,tt),  QD_country(*,regions,*,tt),
QS_region(*,regions,tt),     QD_region(*,regions,tt),
Share_tempS(*,regions,*,tt), ShareS(*,*,tt),
Share_tempD(*,regions,*,tt), ShareD(*,*,tt),
PS_region(*,regions,tt),     PD_region(*,regions,tt)
IM_region(*,regions,tt),     EX_region(*,regions,tt)
Net_trade(k1,regions, tt);
*************************************************************************************************

QS_country(mm,regions, k1,tt)$ mapC2R(mm,regions) =sum(mapC2R(mm,regions),qs1(k1,mm,tt));
QD_country(mm,regions,  f,tt)$ mapC2R(mm,regions) =sum(mapC2R(mm,regions),qd1(f, mm,tt));

QS_region(k1,regions,tt)=sum(mm,QS_country(mm,regions, k1,tt));
QD_region( f,regions,tt)=sum(mm,QD_country(mm,regions,  f,tt));



Share_tempS(mm,regions, k1,tt)$QS_region(k1,regions,tt) = QS_country(mm,regions, k1,tt)/QS_region(k1,regions,tt);
Share_tempD(mm,regions, f ,tt)$QD_region( f,regions,tt) = QD_country(mm,regions,  f,tt)/QD_region( f,regions,tt);

ShareS(mm, k1,tt)= sum(regions,Share_tempS(mm,regions, k1,tt));
ShareD(mm,  f,tt)= sum(regions,Share_tempD(mm,regions,  f,tt));

PS_region(k1,regions,tt)=sum((mapC2R(mm,regions)),PriceS1(k1,mm,tt)*ShareS(mm, k1,tt));
PD_region( f,regions,tt)=sum((mapC2R(mm,regions)),PriceD1( f,mm,tt)*ShareD(mm,  f,tt));

IM_region(k1,regions, tt) = sum((mapC2R(mm,regions)),import(k1,mm,tt));
EX_region(k1,regions, tt) = sum((mapC2R(mm,regions)),export(k1,mm,tt));

Net_trade(k1,regions, tt) = IM_region(k1,regions, tt) - EX_region(k1,regions, tt);




*execute_unload "..\%results%GDX\%SC%.gdx" qd1, qs1, priced1, prices1, Stock, Area, import, export, deltaps, deltaqs, check1, check2, as1,bs1, ad1,bd1,beta, gGDPP, gstock, cornifer, noncornifer,qsRPA, qdRPA, psRPA, pdRPA, importRPA, exportRPA,QS_region,QD_region,PS_region,PD_region,IM_region,EX_region,Net_trade;
*execute 'gdxxrw.exe %results%%SC%.gdx Output=%results%%SC%.xls epsout=0 par=qd1 rng=Demand!a1 par=qs1 rng=Supply!a1 par=priced1 rng=PriceD!a1 par=prices1 rng=PriceS!a1 par=Stock rng=Stock!a1 par=Area rng=Area!a1 par=import rng=Import!a1 par=export rng=Export!a1 par=check1 rng=check1!a1 par=check2 rng=check2!a1 par=as1 rng=interceptS!a1 par=bs1 rng=Slope!a1 par=beta rng=PlantedShare!a1 par=gstock rng=Stockgrowth!a1  par=ad1 rng=Demandintercept!a1 par=bd1 rng=DemandSlope!a1 par=cornifer rng=Charvest!a1 par=noncornifer rng=NCharvest!a1 par=gGDPP rng=gGDPP!a1 par=qsRPA rng=qsRPA!a1 par=qdRPA rng=qdRPA!a1 par=importRPA rng=importRPA!a1 par=exportRPA rng=exportRPA!a1'

*Display gGDPP, qd.l, priced1;

*execute_unload "Results.gdx" qd1, qs1, priced1, prices1;
*execute 'gdxxrw.exe Results.gdx Output=Output.xls epsout=0 par=qd1 rng=Demand!a1 par=qs1 rng=Supply!a1 par=priced1 rng=PriceD!a1 par=prices1 rng=PriceS!a1'
